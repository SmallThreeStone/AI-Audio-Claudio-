...
export default function AIDispatchPanel({
  ...
  return (
    <section className={`ai-dispatch ai-dispatch--${intent?.coverage || 'none'}`}>
      <div className="ai-dispatch__header">
        <div>
          <span>AI 调度台</span>
          <strong>{stage}</strong>
        </div>
        <div className="ai-dispatch__confidence">
          <span>可信度</span>
          <strong>{confidenceLabel(intent?.confidence)}</strong>
        </div>
      </div>
