...
- **AI DJ 控制台层级** — 主路径聚焦一句话开播，素材补给默认折叠为低频工具，减少误触和视觉干扰
- **开播灵感合并** — 常用频道与 AI 此刻建议统一在一个决策区，减少“频道/指令”命名冲突
...
| V7.1.3 | AI DJ 控制台层级收敛 — 主指令只保留心情开播、点名艺人和改变方向；素材补给站默认折叠并移到低频位置；降低场景频道和控制词视觉重量，让开播输入成为唯一主焦点 |
| V7.1.4 | 开播灵感合并 — 将“AI 场景频道”和“AI 场景指令”合并为“开播灵感”，区分常用频道与此刻建议两个来源，减少重复 prompt 区域和用户选择成本 |
| V7.1.5 | 播放控制可见性修复 — 将进度条、暂停、上一首和下一首提升到播放器主路径；右侧播放器区域允许纵向滚动，避免 AI 信息卡过多时控制条被裁掉 |
| V7.1.6 | 主控台常驻与歌单优先修复 — 桌面播放器改为主控优先的紧凑布局，100% 缩放下暂停/上一首/下一首/进度条常驻核心区；“优先我的歌单/只用我的歌单”会排除或限制外部搜索素材，并在 AI 调度台透明展示素材策略 |
| V7.2 | 沉浸播放页与进度控制修复 — 主页面回归 AI DJ 工作台，歌词移入全屏沉浸播放页；播放页左侧展示轮盘、操作区和进度条，右侧常驻滚动歌词；暂停和点击进度条优先控制底层音频节点，避免当前歌曲从头重播 |
